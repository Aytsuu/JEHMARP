...
const initializedFlag = "settingsPrivacyInitialized";
const savingFlag = "settingsPrivacySaving";

function getCardRoot(form: HTMLFormElement) {
  return form.closest<HTMLElement>("[data-admin-settings-privacy]");
}
...
export function initAdminSettingsPrivacy(root: ParentNode = document) {
  const form = root.querySelector<HTMLFormElement>("[data-settings-privacy-form]");
  ...
  form.addEventListener("submit", async (event) => {
    ...
    payload.set("action", "save-platform-settings-privacy");
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error ?? "Unable to save privacy notice settings.");
      }

      setStatus(form, "Saved");
      window.location.reload();
    } catch (error) {
      setStatus(form, error instanceof Error ? error.message : "Unable to save privacy notice settings.", true);
    } finally {
