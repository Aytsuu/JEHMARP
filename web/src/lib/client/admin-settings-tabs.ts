const tabsInitializedPathKey = "adminSettingsTabsInitPath";
const tabParamName = "tab";
const defaultTab = "general";

function getTabTriggers(container: ParentNode) {
  return Array.from(
    container.querySelectorAll<HTMLElement>("[data-admin-settings-tab-trigger]"),
  );
}

function getTabPanels(container: ParentNode) {
  return Array.from(
    container.querySelectorAll<HTMLElement>("[data-admin-settings-tab-panel]"),
  );
}
...
function setActiveTab(
  ...
  panels.forEach((panel) => {
    const isActive = panel.dataset.adminSettingsTabPanel === target;
...
export function initAdminSettingsTabs(root: ParentNode = document) {
  const currentPath = window.location.pathname;

  root.querySelectorAll<HTMLElement>("[data-admin-settings-page]").forEach((container) => {
