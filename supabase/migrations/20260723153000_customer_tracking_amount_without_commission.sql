-- Customer-facing balances exclude agent commission deductions.
...
revoke all on function public.compute_customer_amount_due(uuid) from public;
grant execute on function public.compute_customer_amount_due(uuid) to service_role;
...
begin
  ...
  select
    coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'amountDue', coalesce(public.compute_customer_amount_due(customer_order.id), 0),
    ...
    coalesce(
      sum(coalesce(public.compute_customer_amount_due(customer_order.id), 0)),
      0
    )
