...
-- forward-repair: supabase/migrations/20260804000008_convert_personal_order_to_distribution.sql
...
begin
  if acting_user_id is null then
    raise exception 'An authenticated agent is required to convert personal orders.';
  end if;
  ...
  if current_agent_id is null then
    raise exception 'Only active agents can convert personal orders.';
  end if;
  ...
  if source_order.id is null then
    if exists (
      ...
      raise exception 'Personal order was already converted to a distribution order.';
    end if;

    raise exception 'Personal order was not found.';
  end if;
  ...
  if source_order.parent_order_id is not null then
    raise exception 'Personal order is already linked to a distribution order.';
  end if;

  if source_order.order_status = 'closed' then
    raise exception 'Closed personal orders cannot be converted.';
  end if;

  if source_order.payment_status <> 'unpaid' then
    raise exception 'Only unpaid personal orders can be converted.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Personal orders with payment records cannot be converted.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Personal orders with agent payment records cannot be converted.';
  end if;
  ...
  if item_count = 0 then
    raise exception 'Personal order has no items to convert.';
  end if;
  ...
  set order_kind = 'distribution',
      ...
      notes = nullif(
        concat_ws(
          E'\n\n',
          source_order.notes,
          'Converted from personal order ' || source_order.id::text
        ),
        ''
      ),
...
comment on function public.convert_personal_order_to_distribution_order(uuid) is
  'Agent-only in-place conversion of an eligible personal order into a distribution order. Orders with payment records are blocked.';
