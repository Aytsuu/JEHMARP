...
-- forward-repair: supabase/migrations/20260804000010_agent_conversion_pending_order_status.sql

-- Agent-initiated personal/customer order conversion should await admin approval
-- (pending_order), matching submit_agent_order. Admin conversion remains auto-approved
-- to pending_customers via convert_customer_order_to_agent_distribution_order.
...
comment on function public.convert_personal_order_to_distribution_order(uuid) is
  'Agent-only in-place conversion of an eligible standalone customer or personal agent order into a distribution order awaiting admin approval (pending_order). Orders with payment records are blocked.';
